...
void process(string tranid,vector<string> oneTrans)
{
    ...
    r1=1.0;r2=1.5;r3=5.0;r4=10.0;r5=20.0;
    if(SampleSize < 10)
    {
    ...
    if(id_info_map.find(tranid) != id_info_map.end() )//&& id_cov_map.find(tran_id) != id_cov_map.end())
    {
	    ...
	    if(SampleSize>20 && SEED>=1 && ti.seed_sample_number < 0.1*SampleSize) return;
	    if(SampleSize>20 && ti.Nmj_number >=1) return;
	    if(SampleSize>20 && ti.normal_edge_number<=1)return;
